...
try:
    ...
    if type_ == "code":
        ...
        if not is_ok:
            ...
            cs.execute(f"UPDATE {utl.countries} SET occupied = occupied + 1 WHERE id={row_mbots["country_id"]}")
            ...
            text=f"✅ New account received:\n\n"
            f"🔻 User: <code>{row_mbots['creator_user_id']}</code>\n"
            f"🔻 Number: <code>{row_mbots['phone']}</code>\n"
